...
- **Backend** (`backend/src/routes/primers.rs`):
  ...
  - `POST /api/primers/generate` `{topic, source?}` → slugify topic; return the cached primer if the slug exists; else call `openai::chat_json` (same model constant value as the blindspot generator, temperature 0.2 — factual content) with a system prompt requesting JSON `{title, content_md}`, then run a second adversarial **verification pass** (temperature 0.0) that fact-checks every claim in the draft and returns the corrected `content_md` (falls back to the draft with a logged warning if the verifier flakes). Added 2026-07-08 after a fact audit found the mnemonic section confabulating groupings (Mozart as "Italian Giant", 13 Olympians, a garbled monarch rhyme); the generation prompt also forbids mnemonics that introduce facts absent from the primer's own canon section and forbids reciting traditional verses from memory: a 1,500–2,500-word markdown study guide containing (a) how the topic shows up in Jeopardy clues, (b) the core canon as lists/tables (e.g., composer–work–plot–famous-aria for opera), (c) common clue angles and pivot words, (d) mnemonic hooks, (e) 10 sample clue→response pairs. Insert with `ON CONFLICT (slug) DO NOTHING` then re-select (concurrent-generation guard). `BadRequest` when the API key is unconfigured.
